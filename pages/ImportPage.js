...
class ImportPage {
    ...
    async clickImportPostButton() {
        ...
        if (this.currentPlatform === 'Facebook Reviews') {
            ...
            // 2. Wait for either "Select All" checkbox OR "No reviews found :(" message
            const selectAll = this.page.locator(config.selectAllCheckbox).first();
            const noReviewsMsg = this.page.locator('text="No reviews found :("').first();

            console.log('Waiting for reviews to load or "No reviews found" message...');

            try {
                // Wait for either result
                const result = await Promise.race([
                    selectAll.waitFor({ state: 'visible', timeout: 60000 }).then(() => 'success'),
                    noReviewsMsg.waitFor({ state: 'visible', timeout: 60000 }).then(() => 'no_reviews')
                ]);

                if (result === 'no_reviews' || await noReviewsMsg.isVisible()) {
                    console.log('DETECTED: No reviews found :( - Following instruction to close modal and navigate to Pages link.');
                    await this.closeImportModal();
                    await this.clickPagesLink();

                    // Throw a specific error or set a flag to skip verification steps
                    throw new Error('No reviews found for Facebook');
                }

                // 3. Fallthrough to regular selection and import
                await selectAll.click();
                const finalBtn = this.page.locator(config.finalImportBtn).first();
                await finalBtn.waitFor({ state: 'visible', timeout: 30000 });
                await finalBtn.click();
            } catch (e) {
                if (e.message === 'No reviews found for Facebook') {
                    throw e; // Escalating to step definition to handle skipping
                }
                console.log(`Facebook Import Issue: ${e.message}. Proceeding...`);
            }
    ...
    async getReviewsCount(platformName) {
        console.log(`Getting review count for ${platformName}`);
        // Try to find a number in the success message
        // Example msg: "12 reviews successfully imported"
        let selector = 'text=successfully imported';
        if (this.currentPlatform && this.platformConfig[this.currentPlatform] && this.platformConfig[this.currentPlatform].successMsg) {
            selector = this.platformConfig[this.currentPlatform].successMsg;
        }
        ...
        try {
            ...
            console.log(`Could not extract review count for ${platformName}: ${e.message}`);
        }
